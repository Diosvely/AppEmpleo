...
def main() -> int:
    ...
    momento = datetime.now(timezone.utc)
    sal = perfil["salario"]
    # El salario real vive en un secreto, no en el repositorio publico.
    # Si no esta definido se usa el valor de ejemplo de profile.yaml.
    neto_minimo = float(os.environ.get("SALARIO_NETO_MINIMO") or sal["neto_mensual_minimo"])
    umbral_bruto = nz.neto_mensual_a_bruto_anual(
        neto_minimo, sal["pagas_anuales"], sal["factor_neto_a_bruto"]
    )
